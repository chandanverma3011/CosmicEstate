...
const PropertyFilters = ({ filters, onFilterChange, onApply, onReset, isOpen, onClose }) => {
  ...
  const FilterSection = ({ title, isExpanded, onToggle, children }) => (
    <div className="border-b border-primary/10 last:border-0">
      ...
      >
        <span className="font-semibold text-primary group-hover:text-primary transition-colors">{title}</span>
        <motion.div
  ...
  const filterContent = (
    <>
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold text-primary flex items-center gap-2 drop-shadow-[0_0_10px_rgba(112,0,255,0.3)]">
          <SlidersHorizontal className="w-5 h-5 text-primary" />
          Filters
          {activeFilterCount() > 0 && (
            <span className="w-6 h-6 bg-gradient-to-br from-primary to-secondary rounded-full flex items-center justify-center text-xs font-bold text-white shadow-neon-blue animate-cosmic-glow">
              {activeFilterCount()}
            </span>
          )}
        </h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="select-cosmic w-full px-4 py-3 bg-black/40 border border-primary/20 rounded-xl focus:outline-none focus:border-primary focus:shadow-neon-blue transition-all text-white appearance-none cursor-pointer hover:border-primary/40"
              >
        ...
        >
          <div className="space-y-4">
            <div className="flex justify-between text-sm font-semibold">
              <span className="text-primary drop-shadow-[0_0_8px_rgba(112,0,255,0.4)]">{formatBudget(filters.budgetMin || 1000000)}</span>
              <span className="text-primary drop-shadow-[0_0_8px_rgba(112,0,255,0.4)]">{formatBudget(filters.budgetMax || 100000000)}</span>
            </div>
              ...
              >
                <div className="relative">
                  <input
                    ...
                    className="w-5 h-5 appearance-none border-2 border-primary/30 rounded checked:bg-gradient-to-br checked:from-primary checked:to-secondary checked:border-primary transition-all cursor-pointer shadow-none checked:shadow-neon-blue"
                  />
        ...
        >
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className={`py-3 px-2 rounded-xl font-bold transition-all text-sm ${(filters.bedrooms || 'all') === 'all'
                  ? 'bg-gradient-to-r from-primary to-secondary text-white shadow-neon-blue'
            ...
            {bedroomOptions.map(bedroom => (
              <button
                ...
                className={`py-3 px-2 rounded-xl font-bold transition-all text-sm ${filters.bedrooms === bedroom
                    ? 'bg-gradient-to-r from-primary to-secondary text-white shadow-neon-blue'
                    : 'bg-black/30 text-gray-400 hover:bg-primary/10 hover:text-primary border border-primary/20 hover:border-primary/40'
                  }`}
              >
        ...
        >
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className={`py-3 px-2 rounded-xl font-bold transition-all text-sm ${(filters.bathrooms || 'all') === 'all'
                  ? 'bg-gradient-to-r from-primary to-secondary text-white shadow-neon-blue'
                  : 'bg-black/30 text-gray-400 hover:bg-primary/10 hover:text-primary border border-primary/20 hover:border-primary/40'
                }`}
            >
            ...
            {bathroomOptions.map(bathroom => (
              <button
                ...
                className={`py-3 px-2 rounded-xl font-bold transition-all text-sm ${filters.bathrooms === bathroom
                    ? 'bg-gradient-to-r from-primary to-secondary text-white shadow-neon-blue'
                    : 'bg-black/30 text-gray-400 hover:bg-primary/10 hover:text-primary border border-primary/20 hover:border-primary/40'
                  }`}
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-black/40 border border-primary/20 rounded-xl focus:outline-none focus:border-primary focus:shadow-neon-blue transition-all text-white placeholder-gray-600 hover:border-primary/40"
              />
            </div>
            <div>
      ...
      <div className="mt-8 pt-6 border-t border-primary/20">
        <div className="space-y-3">
          <motion.button
            whileHover={{
              scale: 1.02,
              boxShadow: "0 0 30px rgba(112, 0, 255, 0.6)"
            }}
            whileTap={{ scale: 0.98 }}
            onClick={onApply}
            className="w-full py-4 bg-gradient-to-r from-primary to-secondary rounded-xl text-white font-bold shadow-neon-blue hover:shadow-neon-purple transition-all duration-300"
          >
            Apply Filters
          </motion.button>
          <button
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block lg:w-80 flex-shrink-0">
        <div className="sticky top-24 backdrop-blur-xl bg-black/60 border border-primary/20 rounded-3xl shadow-neon-blue flex flex-col max-h-[calc(100vh-120px)]">
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="absolute left-0 top-0 bottom-0 w-80 bg-black/95 backdrop-blur-xl border-r border-primary/20 shadow-neon-blue flex flex-col"
            >
              <div className="p-6 overflow-y-auto scrollbar-hide flex-1">
                {filterContent}
              </div>
